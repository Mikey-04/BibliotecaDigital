...
if (!in_array($seccion, $permisos_por_rol[$rol_usuario])) {
    ...
    header('Location: panel_admin.php?seccion=inicio&error=acceso_denegado');
    exit;
}
...
if (isset($_GET['solicitud_adquirida'])) {
    $mensaje_global = "✅ La solicitud fue retirada de Libros inexistentes. Completa los datos para registrar el libro en el inventario.";
}

if (isset($_GET['solicitud_error'])) {
    $mensaje_global = $_GET['solicitud_error'] === '1'
        ? "❌ No se encontró la solicitud seleccionada."
        : "❌ No fue posible retirar la solicitud de la lista de pendientes.";
}
...
// --- ACCIÓN: ADQUIRIR UNA SOLICITUD Y RETIRARLA DE PENDIENTES ---
if (isset($_GET['accion']) && $_GET['accion'] === 'adquirir_solicitud' && isset($_GET['id'])) {
    $idSolicitud = (int)$_GET['id'];
    $solicitudSeleccionada = null;

    if ($idSolicitud > 0) {
        foreach ($controladorReservas->obtenerSolicitudesAdquisicion() as $solicitud) {
            if ((int)$solicitud['id'] === $idSolicitud) {
    ...
    if (!$solicitudSeleccionada) {
        header('Location: panel_admin.php?seccion=libros_inexistentes&solicitud_error=1');
        exit;
    }

    $resultadoEliminacion = method_exists($controladorReservas, 'eliminarSolicitud')
        ? $controladorReservas->eliminarSolicitud($idSolicitud)
        : false;

    $eliminada = is_array($resultadoEliminacion)
        ? (bool)($resultadoEliminacion['exito'] ?? false)
        : (bool)$resultadoEliminacion;

    if (!$eliminada) {
        header('Location: panel_admin.php?seccion=libros_inexistentes&solicitud_error=2');
        exit;
    }

    $parametros = http_build_query([
        'seccion' => 'libros',
        'pre_titulo' => $solicitudSeleccionada['nombre_libro'] ?? '',
        'pre_desc' => 'Adquirido por solicitud de ' .
            ($solicitudSeleccionada['primer_nombre'] ?? '') . ' ' .
            ($solicitudSeleccionada['primer_apellido'] ?? ''),
        'pre_categoria' => $solicitudSeleccionada['area'] ?? '',
        'solicitud_adquirida' => 1
    ]);

    header('Location: panel_admin.php?' . $parametros);
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 2. Crear Estudiante con credenciales de acceso (Solo Admin)
    if ($rol_usuario === 'admin' && isset($_POST['crear_estudiante'])) {
        $cip = Validador::sanitizarCadena($_POST['cip'] ?? '');
        $primerNombre = Validador::sanitizarCadena($_POST['p_nombre'] ?? '');
        $primerApellido = Validador::sanitizarCadena($_POST['p_apellido'] ?? '');
        $fechaNacimiento = $_POST['f_nac'] ?? '';
        $carreraId = (int)($_POST['carrera_id'] ?? 0);
        $passwordEstudiante = $_POST['password_estudiante'] ?? '';

        if ($cip === '' || $primerNombre === '' || $primerApellido === '' || $fechaNacimiento === '' || $carreraId <= 0) {
            $mensaje_global = "❌ Todos los datos del estudiante son obligatorios.";
        } elseif (strlen($passwordEstudiante) < 6) {
            $mensaje_global = "❌ La contraseña del estudiante debe tener al menos 6 caracteres.";
        } else {
            try {
                $bd = BaseDatos::obtenerInstancia();
                $stmt = $bd->prepare(
                    "INSERT INTO estudiantes
                        (cip_identificacion, primer_nombre, primer_apellido, fecha_nacimiento, carrera_id, password)
                     VALUES
                        (:cip, :primer_nombre, :primer_apellido, :fecha_nacimiento, :carrera_id, :password)"
                );
                $stmt->execute([
                    ':cip' => $cip,
                    ...
                    ':password' => password_hash($passwordEstudiante, PASSWORD_BCRYPT)
                ]);
                $mensaje_global = "✅ Estudiante registrado correctamente. Ya puede iniciar sesión con su CIP/cédula y contraseña.";
            } catch (\PDOException $e) {
                if ($e->getCode() == 23000 || strpos($e->getMessage(), '1062') !== false) {
                    $mensaje_global = "⚠️ Ya existe un estudiante registrado con ese CIP o cédula.";
                } else {
                    $mensaje_global = "❌ No se pudo registrar el estudiante: " . $e->getMessage();
                }
            }
        }
    }

    // 2.1 Restablecer contraseña de un estudiante existente (Solo Admin)
    if ($rol_usuario === 'admin' && isset($_POST['actualizar_password_estudiante'])) {
        $estudianteId = (int)($_POST['estudiante_id_password'] ?? 0);
        $nuevaPassword = $_POST['nueva_password_estudiante'] ?? '';

        if ($estudianteId <= 0) {
            $mensaje_global = "❌ Selecciona un estudiante válido.";
        } elseif (strlen($nuevaPassword) < 6) {
            $mensaje_global = "❌ La nueva contraseña debe tener al menos 6 caracteres.";
        } else {
            try {
                $bd = BaseDatos::obtenerInstancia();
                $stmt = $bd->prepare("UPDATE estudiantes SET password = :password WHERE id = :id");
                $stmt->execute([
                    ':password' => password_hash($nuevaPassword, PASSWORD_BCRYPT),
                    ':id' => $estudianteId
                ]);
                $mensaje_global = $stmt->rowCount() > 0
                    ? "✅ Contraseña del estudiante actualizada correctamente."
                    : "⚠️ No se encontró el estudiante o la contraseña no cambió.";
            } catch (\PDOException $e) {
                $mensaje_global = "❌ No se pudo actualizar la contraseña: " . $e->getMessage();
            }
        }
    }
...
$categoriasLista = $controladorLibros->obtenerCategorias();
...
<body class="admin-page">
    ...
    <header class="menu-horizontal">
        <div class="admin-brand">
            ...
            <h2>Panel Administrativo <span class="rol-badge"><?php echo htmlspecialchars($rol_usuario); ?></span></h2>
        </div>
        <div class="admin-user">
            <span>👤 <?php echo htmlspecialchars($_SESSION['usuario_nombre']); ?></span>
            <a href="panel_admin.php?seccion=salir">Cerrar sesión</a>
        </div>
    </header>

    <div class="contenedor-central">
        <ul class="tabs">
            <li class="<?php echo $seccion==='inicio'?'activo':''; ?>"><a href="panel_admin.php?seccion=inicio">Inicio</a></li>
            
            <?php if ($rol_usuario === 'admin'): ?>
                <li class="<?php echo $seccion==='usuarios'?'activo':''; ?>"><a href="panel_admin.php?seccion=usuarios">Usuarios</a></li>
                <li class="<?php echo $seccion==='estudiantes'?'activo':''; ?>"><a href="panel_admin.php?seccion=estudiantes">Estudiantes</a></li>
            <?php endif; ?>
            
            <li class="<?php echo $seccion==='libros'?'activo':''; ?>"><a href="panel_admin.php?seccion=libros">Libros</a></li>
            
            <?php if ($rol_usuario === 'admin'): ?>
                <li class="<?php echo $seccion==='profesores'?'activo':''; ?>"><a href="panel_admin.php?seccion=profesores">Profesores</a></li>
            <?php endif; ?>
            
            <li class="<?php echo $seccion==='libros_inexistentes'?'activo':''; ?>"><a href="panel_admin.php?seccion=libros_inexistentes">Libros Inexistentes 📋</a></li>
        </ul>
            ...
            <?php if($seccion === 'inicio'): ?>
                <div style="display: flex; gap: 25px; align-items: flex-start; flex-wrap: wrap;">
                    <div style="flex: 1; min-width: 300px;">
                        ...
                        <br>
                        <a href="panel_admin.php?seccion=libros_inexistentes" style="display: inline-block; padding: 10px 15px; background: #0f172a; color: white; text-decoration: none; border-radius: 6px; font-weight: bold; font-size: 14px;">
                            ...
                            <tr>
                                ...
                                <td>
                                    <?php if($u['username'] !== 'admin'): ?>
                                        <a href="panel_admin.php?seccion=usuarios&accion=eliminar_usuario&id=<?php echo $u['id']; ?>" 
                                           class="btn-eliminar" 
                ...
                <form method="POST" autocomplete="off">
                    <div class="grid-form">
                        ...
                        <div class="campo">
                            ...
                            <input type="password" name="password_estudiante" minlength="6" required autocomplete="new-password" placeholder="Mínimo 6 caracteres">
                        </div>
                        <div class="campo">
                            <label>Carrera</label>
                            <select name="carrera_id" required>
                                <?php foreach($controladorEstudiantes->obtenerCarreras() as $c): ?>
                                    <option value="<?php echo $c['id']; ?>"><?php echo htmlspecialchars($c['nombre_carrera']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <button type="submit" name="crear_estudiante">💾 Guardar Estudiante y Crear Acceso</button>
                </form>

                <h4>Restablecer contraseña de un estudiante existente</h4>
                <form method="POST" autocomplete="off">
                    <div class="grid-form">
                        <div class="campo">
                            <label>Estudiante</label>
                            <select name="estudiante_id_password" required>
                                <option value="">-- Seleccionar Estudiante --</option>
                                <?php foreach($estudiantesLista as $e): ?>
                                    <option value="<?php echo $e['id']; ?>">
                                        <?php echo htmlspecialchars("[{$e['cip_identificacion']}] {$e['primer_nombre']} {$e['primer_apellido']}"); ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="campo">
                            ...
                            <input type="password" name="nueva_password_estudiante" minlength="6" required autocomplete="new-password" placeholder="Mínimo 6 caracteres">
                        </div>
                    </div>
                    <button type="submit" name="actualizar_password_estudiante">🔐 Actualizar contraseña</button>
                </form>

                <h4>Estudiantes Matriculados</h4>
                <table class="tabla">
                    <thead><tr><th>Cédula</th><th>Nombre</th><th>Carrera</th><th>Acceso</th></tr></thead>
                    <tbody>
                        <?php foreach($estudiantesLista as $e): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($e['cip_identificacion']); ?></td>
                                <td><?php echo htmlspecialchars($e['primer_nombre'].' '.$e['primer_apellido']); ?></td>
                                <td><?php echo htmlspecialchars($e['nombre_carrera']); ?></td>
                                <td><span class="rol-badge" style="background:#0d9488;">Habilitado</span></td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
            ...
            <?php elseif($seccion === 'libros'): ?>
                <?php if ($libroEditar): ?>
                    ...
                    <form method="POST" enctype="multipart/form-data" style="border: 2px solid #eab308; background: #fefcf0;">
                        ...
                        <a href="panel_admin.php?seccion=libros" style="margin-left:10px; text-decoration:none; color:#555; font-weight:bold;">Cancelar</a>
                    </form>
                <?php else: ?>
                    ...
                    <form method="POST" enctype="multipart/form-data">
                        <div class="grid-form">
                            ...
                            <div class="campo">
                                ...
                                <select name="categoria_id" required>
                                    <?php foreach($controladorLibros->obtenerCategorias() as $cat): ?>
                                        <option value="<?php echo $cat['id']; ?>" <?php echo (isset($_GET['pre_categoria']) && $_GET['pre_categoria'] === $cat['nombre']) ? 'selected' : ''; ?>>
                    ...
                    <tbody>
                        <?php foreach($librosLista as $l): ?>
                            <tr>
                                ...
                                <td>
                                    <a href="panel_admin.php?seccion=libros&editar_libro_id=<?php echo $l['id']; ?>" class="btn-editar">✏️ Editar</a>
                                    <a href="panel_admin.php?seccion=libros&accion=eliminar_libro&id=<?php echo $l['id']; ?>" class="btn-eliminar" onclick="return confirm('¿Seguro que deseas eliminar permanentemente este libro?');">🗑️ Eliminar</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    </tbody>
                        ...
                        <?php else: ?>
                            <?php foreach($profesoresLista as $p): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="panel_admin.php?seccion=profesores&accion=eliminar_profesor&id=<?php echo $p['id']; ?>" class="btn-eliminar" onclick="return confirm('¿Seguro que deseas eliminar permanentemente a este profesor?');">🗑️ Eliminar</a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        <?php endif; ?>
            ...
            <?php elseif($seccion === 'libros_inexistentes'): ?>
                ...
                <form method="POST">
                    ...
                    <div class="grid-form">
                        ...
                        <div class="campo">
                            <label>Categoría / Área Temática</label>
                        ...
                        <?php else: ?>
                            <?php foreach ($solicitudes as $sol): ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="panel_admin.php?seccion=libros_inexistentes&accion=adquirir_solicitud&id=<?php echo (int)$sol['id']; ?>"
                                           class="btn-accion-rapida"
                                           title="Retirar de pendientes y abrir el formulario de registro"
                                           onclick="return confirm('La solicitud dejará de aparecer como pendiente y se abrirá el formulario para registrar el libro. ¿Continuar?');">
                                           🛒 Adquirir y Registrar
                                        </a>
                                        
                                        <a href="panel_admin.php?seccion=libros_inexistentes&accion=eliminar_solicitud&id=<?php echo $sol['id']; ?>" 
                                           class="btn-eliminar" 
